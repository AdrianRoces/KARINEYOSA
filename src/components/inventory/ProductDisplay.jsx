      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4 justify-center px-1 sm:px-2">
        {products.map((product) => {
          ...
          return (
            <div
              key={product.id}
              className={`w-full rounded-[28px] border border-gray-300 shadow-[0_14px_24px_rgba(101,54,111,0.18)] px-4 py-4 transition-all duration-300 transform group hover:-translate-y-1 ${
              ...
              <div className="flex flex-col gap-4">
                <div className="overflow-hidden rounded-[28px] border border-[#65366F]/10 shadow-sm">
                  <div className="relative aspect-[3/4] w-full overflow-hidden">
                    ...
                    {showEditMode && (
                      <div className="absolute top-3 right-3 rounded-full bg-white/90 px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#841c4f] shadow-sm">
                        Edit
                      </div>
                    )}
                    <div className="absolute left-3 right-3 top-3 flex justify-center transition-opacity duration-300 group-hover:opacity-0">
                      <div className="text-center pointer-events-none rounded-[18px] bg-black/70 px-3 py-2 shadow-[0_6px_18px_rgba(0,0,0,0.18)]">
                        <div className="text-base sm:text-lg font-semibold text-white leading-tight truncate">
                          {product.name}
                        </div>
                        <div className="text-[11px] sm:text-xs uppercase tracking-[0.12em] text-white/90 leading-tight mt-1 truncate">
                          {product.variantName || product.variant_name || 'No variant'}
                        </div>
                      </div>
                    </div>
                    <div className="absolute inset-0 flex items-end p-3 opacity-0 transition-all duration-300 group-hover:opacity-100">
                      <div className="w-full rounded-[20px] bg-black/70 p-3 text-white backdrop-blur-sm">
                        <div className="text-base sm:text-lg font-semibold truncate">{product.name}</div>
                        <div className="mt-1 text-[11px] sm:text-xs uppercase tracking-[0.18em] text-[#d8c7ff] truncate">
                          {product.category || 'Uncategorized'}
                        </div>
                        <div className="mt-3 text-lg sm:text-xl font-semibold">₱{Number(product.price || 0).toFixed(2)}</div>
